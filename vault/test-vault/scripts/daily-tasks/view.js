...
const tagsMap = {};
const allowedTags = Object.values(tags).reduce(
  (ac, cv) => ({ ...ac, [cv]: true }),
  {}
);

const hasStatus = (t, status) => t?.outlinks.some(link => link?.path === status)
...
tasks.forEach((task) => {
  ...
  if (task.start) {
    const diff = Math.round(logDate.diff(task.start, 'days').toObject()?.days);
    let diffText = `==D${diff > 0 ? `+${Math.abs(diff)}`: diff}==`;
    if (diff === 0 ) diffText = '==TODAY==';
    task.visual = `${diffText} ${(task.visual ? task.visual : task.text).replace(/\[start::[\d-]+\]/,'')}`
...
else dv.taskList(tagsMap["noTag"].sort((a,b) => {
  const startA = +(a.start ?? new Date().getTime())
  const startB = +(b.start ?? new Date().getTime())
